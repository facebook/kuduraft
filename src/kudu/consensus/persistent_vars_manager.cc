...
Status PersistentVarsManager::LoadOrCreatePersistentVars(const string& tablet_id,
                                                   PersistentVarsCreateMode create_mode,
                                                   scoped_refptr<PersistentVars>* persistent_vars_out) {
  Status s = LoadPersistentVars(tablet_id, persistent_vars_out);
  if (s.IsNotFound()) {
    return CreatePersistentVars(tablet_id, create_mode, persistent_vars_out);
  }
  return s;
}

Status PersistentVarsManager::DeletePersistentVars(const string& tablet_id) {
  {
    lock_guard<Mutex> l(persistent_vars_lock_);
    persistent_vars_cache_.erase(tablet_id); // OK to delete an uncached persistent_vars; ignore the return value.
  }
  RETURN_NOT_OK_PREPEND(PersistentVars::DeleteOnDiskData(fs_manager_, tablet_id),
