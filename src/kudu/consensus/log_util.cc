...
Status LogEntryReader::readNextEntry(unique_ptr<LogEntryPB>* entry) {
  ...
  while (pendingEntries_.empty()) {
    ...
    unique_ptr<LogEntryBatchPB> currentBatch;

    // Read and validate the entry header first.
    Status s;
    EntryHeaderStatus sDetail = EntryHeaderStatus::OtherError;
    if (offset_ + seg_->entryHeaderSize() < readUpTo_) {
      s = seg_->readEntryHeaderAndBatch(
          &offset_, &tmpBuf_, &currentBatch, &sDetail);
    } else {
      s = Status::Corruption(
    ...
    if (PREDICT_FALSE(!s.ok())) {
      return handleReadError(s, sDetail);
    }

    // Add the entries from this batch to our pending queue.
    for (int i = 0; i < currentBatch->entry_size(); i++) {
      auto currentEntry = currentBatch->mutable_entry(i);
      pendingEntries_.emplace_back(currentEntry);
      numEntriesRead_++;

      // Record it in the 'recent entries' deque.
      OpId opId;
      if (currentEntry->type() == log::REPLICATE &&
          currentEntry->has_replicate()) {
        opId = currentEntry->replicate().id();
      } else if (
          currentEntry->has_commit() &&
          currentEntry->commit().has_commited_op_id()) {
        opId = currentEntry->commit().commited_op_id();
      }
      if (recentEntries_.size() == kNumRecentEntries) {
        recentEntries_.pop_front();
      }
      recentEntries_.push_back({offset_, currentEntry->type(), opId});
    }
    currentBatch->mutable_entry()->UnsafeArenaExtractSubrange(
        0, currentBatch->entry_size(), nullptr);
  }

  *entry = std::move(pendingEntries_.front());
...
Status LogEntryReader::handleReadError(
  ...
  // batch).
  if (seg_->hasFooter() && !seg_->footerWasRebuilt_) {
    LOG(WARNING) << "Found a corruption in a closed log segment: "
                 << corruptionStatus.ToString();
  ...
  RETURN_NOT_OK_PREPEND(
      seg_->scanForValidEntryHeaders(
          offset_ + seg_->entryHeaderSize(), &hasValidEntries),
      "Scanning forward for valid entries");
  if (hasValidEntries) {
    return corruptionStatus;
  }

  CHECK(statusDetail != EntryHeaderStatus::Ok);
  if (statusDetail == EntryHeaderStatus::AllZeros) {
    ...
    LOG(INFO)
        ...
        << "Error detail: " << corruptionStatus.ToString();
  }
...
Status ReadableLogSegment::open(
  ...
  shared_ptr<RandomAccessFile> readableFile;
  RETURN_NOT_OK_PREPEND(
      env_util::openFileForRandom(env, path, &readableFile),
      "Unable to open file for reading");

  *segment = std::shared_ptr<ReadableLogSegment>(
      new ReadableLogSegment(path, readableFile));
...
ReadableLogSegment::ReadableLogSegment(
    ...
    : path_(std::move(path)),
      fileSize_(0),
      readableToOffset_(0),
      readableFile_(std::move(readableFile)),
      codec_(nullptr),
      isInitialized_(false),
      footerWasRebuilt_(false) {}
...
Status ReadableLogSegment::init(
  ...
  firstEntryOffset_ = firstEntryOffset;
  isInitialized_ = true;
  readableToOffset_.store(fileSize());
...
Status ReadableLogSegment::init(
  ...
  readableToOffset_.store(firstEntryOffset);
...
const int64_t ReadableLogSegment::readableUpTo() const {
  return readableToOffset_.load();
}

void ReadableLogSegment::updateReadableToOffset(int64_t readableToOffset) {
  readableToOffset_.store(readableToOffset);
  fileSize_.storeMax(readableToOffset);
}
...
Status ReadableLogSegment::rebuildFooterByScanning() {
  ...
  while (true) {
    ...
    if (entry->has_replicate()) {
      updateFooterForReplicateEntry(*entry, &newFooter);
    }
    numEntries++;
  }

  newFooter.set_num_entries(numEntries);
  footer_ = newFooter;
  DCHECK(footer_.IsInitialized());
  footerWasRebuilt_ = true;
  readableToOffset_.store(reader.offset());
...
Status ReadableLogSegment::readFileSize() {
  ...
  uint64_t size;
  RETURN_NOT_OK_PREPEND(readableFile_->Size(&size), "Unable to read file size");
  fileSize_.store(size);
  if (size == 0) {
...
Status ReadableLogSegment::readHeader() {
  uint32_t headerSize;
  RETURN_NOT_OK(readHeaderMagicAndHeaderLength(&headerSize));

  if (headerSize > kLogSegmentMaxHeaderOrFooterSize) {
    return Status::Corruption(
        fmt::format(
            "File is corrupted. "
  ...
  uint8_t headerSpace[headerSize];
  Slice headerSlice(headerSpace, headerSize);
  LogSegmentHeaderPB header;

  // Read and parse the log segment header.
  RETURN_NOT_OK_PREPEND(
      readableFile_->Read(kLogSegmentHeaderMagicAndHeaderLength, headerSlice),
      "Unable to read fully");

  RETURN_NOT_OK_PREPEND(
      pb_util::ParseFromArray(&header, headerSlice.data(), headerSize),
  ...
  header_.Swap(&header);
  firstEntryOffset_ = headerSize + kLogSegmentHeaderMagicAndHeaderLength;
...
Status ReadableLogSegment::readHeaderMagicAndHeaderLength(uint32_t* len) {
  ...
  RETURN_NOT_OK(readableFile_->Read(0, slice));
...
Status ReadableLogSegment::readFooter() {
  uint32_t footerSize;
  RETURN_NOT_OK(readFooterMagicAndFooterLength(&footerSize));

  if (footerSize == 0 || footerSize > kLogSegmentMaxHeaderOrFooterSize) {
    return Status::Corruption(
        fmt::format(
            "File is corrupted. "
            "Parsed header size: {} is zero or bigger than max header size: {}",
  ...
  if (footerSize > (fileSize() - firstEntryOffset_)) {
    return Status::Corruption(
        "Footer not found. File corrupted. "
        "Decoded footer length pointed at a footer before the first entry.");
  }

  uint8_t footerSpace[footerSize];
  Slice footerSlice(footerSpace, footerSize);

  int64_t footerOffset =
      fileSize() - kLogSegmentFooterMagicAndFooterLength - footerSize;

  LogSegmentFooterPB footer;

  // Read and parse the log segment footer.
  RETURN_NOT_OK_PREPEND(
      readableFile_->Read(footerOffset, footerSlice),
      "Footer not found. Could not read fully.");

  RETURN_NOT_OK_PREPEND(
      pb_util::ParseFromArray(&footer, footerSlice.data(), footerSize),
      "Unable to parse protobuf");
...
Status ReadableLogSegment::readFooterMagicAndFooterLength(uint32_t* len) {
  ...
  CHECK_GT(fileSize(), kLogSegmentFooterMagicAndFooterLength);
  RETURN_NOT_OK(readableFile_->Read(
      fileSize() - kLogSegmentFooterMagicAndFooterLength, slice));
...
size_t ReadableLogSegment::entryHeaderSize() const {
  DCHECK(isInitialized_);
...
Status ReadableLogSegment::scanForValidEntryHeaders(
  ...
  *hasValidEntries = false;
  ...
  for (; offset < fileSize() - entryHeaderSize();
       offset += kChunkSize - entryHeaderSize()) {
    int rem = std::min<int64_t>(fileSize() - offset, kChunkSize);
    Slice chunk(buf.get(), rem);
    RETURN_NOT_OK(readableFile()->Read(offset, chunk));
...
Status ReadableLogSegment::readEntryHeaderAndBatch(
  ...
  EntryHeader header;
  RETURN_NOT_OK(readEntryHeader(&curOffset, &header, statusDetail));
  Status s = readEntryBatch(&curOffset, header, tmpBuf, batch);
  if (PREDICT_FALSE(!s.ok())) {
    // If we failed to actually decode the batch, make sure to set statusDetail
    // to non-Ok.
    *statusDetail = EntryHeaderStatus::OtherError;
    return s;
  }
  *offset = curOffset;
  return Status::OK();
...
Status ReadableLogSegment::readEntryHeader(
    ...
    EntryHeader* header,
    EntryHeaderStatus* statusDetail) {
  const size_t headerSize = entryHeaderSize();
  uint8_t scratch[headerSize];
  Slice slice(scratch, headerSize);
  RETURN_NOT_OK_PREPEND(
      readableFile()->Read(*offset, slice), "Could not read log entry header");

  *statusDetail = decodeEntryHeader(slice, header);
  switch (*statusDetail) {
...
EntryHeaderStatus ReadableLogSegment::decodeEntryHeader(
  ...
  if (entryHeaderSize() == kEntryHeaderSizeV2) {
...
Status ReadableLogSegment::readEntryBatch(
    ...
    faststring* tmpBuf,
    unique_ptr<LogEntryBatchPB>* entryBatch) {
  TRACE_EVENT2(
      "log",
  ...
  tmpBuf->clear();
  size_t bufLen = header.msgLengthCompressed;
  if (codec_) {
    // Reserve some space for the decompressed copy as well.
    bufLen += header.msgLength;
  }
  tmpBuf->resize(bufLen);
  Slice entryBatchSlice(tmpBuf->data(), header.msgLengthCompressed);
  Status s = readableFile()->Read(*offset, entryBatchSlice);
  ...
  // Verify the CRC.
  uint32_t readCrc =
      crc::crc32c(entryBatchSlice.data(), entryBatchSlice.size());
  if (PREDICT_FALSE(readCrc != header.msgCrc)) {
    return Status::Corruption(
        fmt::format(
  ...
  if (codec_) {
    ...
    uint8_t* uncompressBuf = &(*tmpBuf)[header.msgLengthCompressed];
    RETURN_NOT_OK_PREPEND(
        codec_->uncompress(entryBatchSlice, uncompressBuf, header.msgLength),
        "failed to uncompress entry");
    entryBatchSlice = Slice(uncompressBuf, header.msgLength);
  }

  unique_ptr<LogEntryBatchPB> readEntryBatchPb(new LogEntryBatchPB);
  s = pb_util::ParseFromArray(
      readEntryBatchPb.get(), entryBatchSlice.data(), header.msgLength);
  ...
  *offset += header.msgLengthCompressed;
  entryBatch->reset(readEntryBatchPb.release());
  return Status::OK();
}
...
Status WritableLogSegment::writeHeaderAndOpen(
  ...
  DCHECK(!isHeaderWritten()) << "Can only call writeHeaderAndOpen() once";
  DCHECK(newHeader.IsInitialized()) << "Log segment header must be initialized"
                                    << newHeader.InitializationErrorString();
  faststring buf;
...
Status WritableLogSegment::writeEntryBatch(
  ...
  // Fill in the header.
  inlineEncodeFixed32(&headerBuf[0], dataToWrite.size());
  inlineEncodeFixed32(&headerBuf[4], uncompressedLen);
  inlineEncodeFixed32(
      &headerBuf[8], crc::crc32c(dataToWrite.data(), dataToWrite.size()));
  inlineEncodeFixed32(
      &headerBuf[12], crc::crc32c(&headerBuf[0], kEntryHeaderSizeV2 - 4));
...
unique_ptr<LogEntryBatchPB> createBatchFromAllocatedOperations(
  ...
  unique_ptr<LogEntryBatchPB> entryBatch(new LogEntryBatchPB);
  entryBatch->mutable_entry()->Reserve(msgs.size());
  for (const auto& msg : msgs) {
    LogEntryPB* entryPb = entryBatch->add_entry();
    entryPb->set_type(log::REPLICATE);
    entryPb->set_allocated_replicate(msg->get());
  }
  return entryBatch;
}
