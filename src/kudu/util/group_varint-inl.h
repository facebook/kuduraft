...
extern bool sseTableInitted;
extern uint8_t sseTable[256 * 16] __attribute__((aligned(16)));
extern uint8_t varintSelectorLengths[256];

const uint32_t kMasks[4] = {0xff, 0xffff, 0xffffff, 0xffffffff};
...
inline const uint8_t* decodeGroupVarInt32Sse(
    const uint8_t* src,
    uint32_t* a,
    uint32_t* b,
    uint32_t* c,
