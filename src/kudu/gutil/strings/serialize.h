...
inline uint32_t KeyToUint32(const StringPiece& key) {
  uint32_t value;
...
inline uint64_t KeyToUint64(const StringPiece& key) {
  uint64_t value;
  DCHECK_EQ(key.size(), sizeof(value));
  memcpy(&value, key.data(), sizeof(value));
  return ntohll(value);
...
int32_t KeyToInt32(const StringPiece& key);
...
int32_t OrderedStringToInt32(const StringPiece& key);
...
// The inverse of the above function.
int64_t OrderedStringToInt64(const StringPiece& key);
...
// The inverse of the above function.
int32_t ReverseOrderedStringToInt32(const StringPiece& key);
...
// The inverse of the above function.
int64_t ReverseOrderedStringToInt64(const StringPiece& key);
...
inline bool DecodeUint32(const StringPiece& s, uint32_t* i) {
  return DecodePOD(s, i);
}

inline bool DecodeUint64(const StringPiece& s, uint64_t* i) {
  return DecodePOD(s, i);
}
